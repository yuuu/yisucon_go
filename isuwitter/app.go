...
func topHandler(w http.ResponseWriter, r *http.Request) {
	...
	var err error
	if until == "" {
		rows, err = db.Query(`SELECT tweets.id, tweets.user_id, tweets.text, tweets.created_at FROM tweets INNER JOIN (SELECT friend_id FROM friends WHERE friends.user_id=?) AS friends ON tweets.user_id=friends.friend_id ORDER BY tweets.id DESC LIMIT ?`, userID.(int), perPage)
	} else {
